...
class AU_GANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            ...
            type=float,
            default=10.0,
            help="weight for reconstruction losses",
        )
        parser.add_argument(
            "--lambda_identity",
            type=float,
        ...
        parser.add_argument(
            "--lambda_perc", type=float, default=20.0, help="perceptual loss weight"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=float,
            default=1.0,
            help="global-to-instance loss weighting ratio. Default is 1-to-1 weighting.",
        )
        return parser
    ...
    def __init__(self, opt):
        ...
        # Use reconstructor of Night to Day in the Day to Night model
        self.netG_B.model_dec = self.netG_A.model_rec
        self.netG_B.model_rec = self.netG_A.model_dec
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_rec_A = networks.define_D(
                opt.output_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_rec_B = networks.define_D(
                opt.input_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_rec_fake_A = networks.define_D(
                opt.output_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.netD_rec_fake_B = networks.define_D(
                opt.input_nc,
                opt.ndf,
                opt.netD,
                opt.n_layers_D,
            ...
            self.optimizer_D_rec = torch.optim.Adam(
                itertools.chain(
                    self.netD_rec_A.parameters(), self.netD_rec_B.parameters()
                ),
                lr=opt.lr,
                betas=(opt.beta1, 0.999),
                eps=1e-4,
            )
            self.optimizer_D_rec_fake = torch.optim.Adam(
                itertools.chain(
                    self.netD_rec_fake_A.parameters(), self.netD_rec_fake_B.parameters()
                ),
                lr=opt.lr,
                betas=(opt.beta1, 0.999),
                eps=1e-4,
            )
            self.optimizer_DC = torch.optim.Adam(
                self.netDC.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999), eps=1e-4
            )
            ...
            if opt.instance_level:
                ...
                # Discriminators
                self.netD_A_inst = networks.define_D(
                    opt.output_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                self.netD_B_inst = networks.define_D(
                    opt.output_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                self.netD_rec_A_inst = networks.define_D(
                    opt.output_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                self.netD_rec_B_inst = networks.define_D(
                    opt.input_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                self.netD_rec_fake_A_inst = networks.define_D(
                    opt.output_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                self.netD_rec_fake_B_inst = networks.define_D(
                    opt.input_nc,
                    opt.ndf,
                    "basic",
                    opt.n_layers_D,
                ...
                # Optimizers
                self.optimizer_D_inst = torch.optim.Adam(
                    itertools.chain(
                        self.netD_A_inst.parameters(), self.netD_B_inst.parameters()
                    ),
                    lr=opt.lr,
                    betas=(opt.beta1, 0.999),
                    eps=1e-4,
                )
                self.optimizer_D_rec_inst = torch.optim.Adam(
                    itertools.chain(
                        self.netD_rec_A_inst.parameters(),
                        self.netD_rec_B_inst.parameters(),
                    ),
                    lr=opt.lr,
                    betas=(opt.beta1, 0.999),
                    eps=1e-4,
                )
                self.optimizer_D_rec_fake_inst = torch.optim.Adam(
                    itertools.chain(
                        self.netD_rec_fake_A_inst.parameters(),
                        self.netD_rec_fake_B_inst.parameters(),
                    ),
                    lr=opt.lr,
                    betas=(opt.beta1, 0.999),
                    eps=1e-4,
                ...
                self.optimizers += [
                    self.optimizer_D_inst,
                    self.optimizer_D_rec_inst,
                    self.optimizer_D_rec_fake_inst,
                ]
        ...
        if opt.distributed:
            self.netG_A = torch.nn.parallel.DistributedDataParallel(
                self.netG_A,
                device_ids=[opt.gpu],
                output_device=opt.gpu,
                find_unused_parameters=True,
            )
            self.netG_B = torch.nn.parallel.DistributedDataParallel(
                self.netG_B,
                device_ids=[opt.gpu],
                output_device=opt.gpu,
                find_unused_parameters=True,
            ...
            if opt.isTrain:
                self.netD_A = torch.nn.parallel.DistributedDataParallel(
                    self.netD_A,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                self.netD_B = torch.nn.parallel.DistributedDataParallel(
                    self.netD_B,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                self.netD_rec_A = torch.nn.parallel.DistributedDataParallel(
                    self.netD_rec_A,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                self.netD_rec_B = torch.nn.parallel.DistributedDataParallel(
                    self.netD_rec_B,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                self.netD_rec_fake_A = torch.nn.parallel.DistributedDataParallel(
                    self.netD_rec_fake_A,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                self.netD_rec_fake_B = torch.nn.parallel.DistributedDataParallel(
                    self.netD_rec_fake_B,
                    device_ids=[opt.gpu],
                    output_device=opt.gpu,
                    find_unused_parameters=True,
                )
                if opt.instance_level:
                    self.netD_A_inst = torch.nn.parallel.DistributedDataParallel(
                        self.netD_A_inst,
                        device_ids=[opt.gpu],
                        output_device=opt.gpu,
                        find_unused_parameters=True,
                    )
                    self.netD_B_inst = torch.nn.parallel.DistributedDataParallel(
                        self.netD_B_inst,
                        device_ids=[opt.gpu],
                        output_device=opt.gpu,
                        find_unused_parameters=True,
                    )
                    self.netD_rec_A_inst = torch.nn.parallel.DistributedDataParallel(
                        self.netD_rec_A_inst,
                        device_ids=[opt.gpu],
                        output_device=opt.gpu,
                        find_unused_parameters=True,
                    )
                    self.netD_rec_B_inst = torch.nn.parallel.DistributedDataParallel(
                        self.netD_rec_B_inst,
                        device_ids=[opt.gpu],
                        output_device=opt.gpu,
                        find_unused_parameters=True,
                    )
                    self.netD_rec_fake_A_inst = (
    ...
    def forward(self):
        with tamp.autocast():
            ...
            if (
                self.opt.isTrain
                and self.opt.instance_level
                and self.BBoxAvailable(self.bboxes_A)
                and self.BBoxAvailable(self.bboxes_B)
            ):
                # A -> B instances
                self.real_A_inst = self.crop_instances(
                    self.real_A, self.bboxes_A, self.crop_size_A
                )  # A
                self.rec_A_inst = self.crop_instances(
                    self.rec_A, self.bboxes_A, self.crop_size_A
                )  # A -> A
                self.fake_B_inst = self.crop_instances(
                    self.fake_B, self.bboxes_A, self.crop_size_A
                )  # A -> B
                self.rec_fake_B_inst = self.crop_instances(
                    self.rec_fake_B, self.bboxes_A, self.crop_size_A
                )  # A -> B -> B
                self.fake_A__inst = self.crop_instances(
                    self.fake_A_, self.bboxes_A, self.crop_size_A
                )  # A -> B -> A

                # B -> A instances
                self.real_B_inst = self.crop_instances(
                    self.real_B, self.bboxes_B, self.crop_size_B
                )
                self.rec_B_inst = self.crop_instances(
                    self.rec_B, self.bboxes_B, self.crop_size_B
                )
                self.fake_A_inst = self.crop_instances(
                    self.fake_A, self.bboxes_B, self.crop_size_B
                )
                self.rec_fake_A_inst = self.crop_instances(
                    self.rec_fake_A, self.bboxes_B, self.crop_size_B
                )
                self.fake_B__inst = self.crop_instances(
                    self.fake_B_, self.bboxes_B, self.crop_size_B
                )
    ...
    def backward_D_A_fake_rec(self):
        ...
        self.loss_D_A_fake_rec = self.backward_D_basic(
            self.netD_rec_fake_A, self.real_A, rec_fake_A
        )

    def backward_D_B_fake_rec(self):
        rec_fake_B = self.rec_B_fake_pool.query(self.rec_fake_B)
        self.loss_D_B_fake_rec = self.backward_D_basic(
            self.netD_rec_fake_B, self.real_B, rec_fake_B
        )
    ...
    def backward_D_A_inst(self):
        ...
        self.loss_D_A_inst = self.backward_D_basic(
            self.netD_A_inst, self.real_B_inst, fake_B_inst
        )

    def backward_D_B_inst(self):
        fake_A_inst = self.fake_A_pool_inst.query(self.fake_A_inst)
        self.loss_D_B_inst = self.backward_D_basic(
            self.netD_B_inst, self.real_A_inst, fake_A_inst
        )

    def backward_D_A_rec_inst(self):
        rec_A_inst = self.rec_A_pool_inst.query(self.rec_A_inst)
        self.loss_D_rec_A_inst = self.backward_D_basic(
            self.netD_rec_A_inst, self.real_A_inst, rec_A_inst
        )

    def backward_D_B_rec_inst(self):
        rec_B_inst = self.rec_B_pool_inst.query(self.rec_B_inst)
        self.loss_D_rec_B_inst = self.backward_D_basic(
            self.netD_rec_B_inst, self.real_B_inst, rec_B_inst
        )

    def backward_D_A_fake_rec_inst(self):
        rec_fake_A_inst = self.rec_A_fake_pool_inst.query(self.rec_fake_A_inst)
        self.loss_D_A_fake_rec_inst = self.backward_D_basic(
            self.netD_rec_fake_A_inst, self.real_A_inst, rec_fake_A_inst
        )

    def backward_D_B_fake_rec_inst(self):
        rec_fake_B_inst = self.rec_B_fake_pool_inst.query(self.rec_fake_B_inst)
        self.loss_D_B_fake_rec_inst = self.backward_D_basic(
            self.netD_rec_fake_B_inst, self.real_B_inst, rec_fake_B_inst
        )
    ...
    def backward_DC(self):
        with tamp.autocast():
            ...
            self.loss_DC = (
                0.25 * self.criterionCls(self.netDC(self.enc_A.detach()), "A")
                + 0.25 * self.criterionCls(self.netDC(self.enc_B.detach()), "B")
                + 0.25 * self.criterionCls(self.netDC(self.enc_fake_A.detach()), "A")
                + 0.25 * self.criterionCls(self.netDC(self.enc_fake_B.detach()), "B")
            )
    ...
    def backward_G(self):
        with tamp.autocast():
            ...
            self.loss_cycle_A = (
                self.criterionCycle(self.fake_A_, self.real_A) * lambda_A
            )
            # Backward cycle loss || G_A(G_B(B)) - B||
            self.loss_cycle_B = (
                self.criterionCycle(self.fake_B_, self.real_B) * lambda_B
            )
            ...
            self.loss_G_A_fake_rec = self.criterionGAN(
                self.netD_rec_fake_A(self.rec_fake_A), True
            )
            self.loss_G_B_fake_rec = self.criterionGAN(
                self.netD_rec_fake_B(self.rec_fake_B), True
            )
            ...
            self.loss_G_DC = lambda_dc * (
                0.25 * self.criterionCls(self.netDC(self.enc_A), "B")
                + 0.25 * self.criterionCls(self.netDC(self.enc_fake_A), "B")
                + 0.25 * self.criterionCls(self.netDC(self.enc_B), "A")
                + 0.25 * self.criterionCls(self.netDC(self.enc_fake_B), "A")
            )
            self.loss_Perc = (
                self.criterionPerceptual(self.enc_A, self.enc_fake_B)
            ...
            self.loss_G = (
                ...
                + self.loss_G_A_rec
                + self.loss_G_B_rec
                + self.loss_G_A_fake_rec
                + self.loss_G_B_fake_rec
                + self.loss_A_rec
                + self.loss_B_rec
                + self.loss_G_DC
                + self.loss_Perc
            ...
            if (
                ...
                self.loss_G_A_inst = self.criterionGAN(
                    self.netD_A_inst(self.fake_B_inst), True
                )
                self.loss_G_B_inst = self.criterionGAN(
                    self.netD_B_inst(self.fake_A_inst), True
                )
                self.loss_G_A_rec_inst = self.criterionGAN(
                    self.netD_A_inst(self.rec_A_inst), True
                )
                self.loss_G_B_rec_inst = self.criterionGAN(
                    self.netD_B_inst(self.rec_B_inst), True
                )
                self.loss_G_A_fake_rec_inst = self.criterionGAN(
                    self.netD_A_inst(self.rec_fake_A_inst), True
                )
                self.loss_G_B_fake_rec_inst = self.criterionGAN(
                    self.netD_B_inst(self.rec_fake_B_inst), True
                )
                # Cycle Loss
                self.loss_cycle_A_inst = (
                    self.criterionCycle(self.fake_A__inst, self.real_A_inst) * lambda_A
                )
                self.loss_cycle_B_inst = (
                    self.criterionCycle(self.fake_B__inst, self.real_B_inst) * lambda_B
                )
                # L1 reconstruction loss
                self.loss_A_rec_inst = (
                    self.criterionRec(self.rec_A_inst, self.real_A_inst) * lambda_rec
                )
                self.loss_B_rec_inst = (
                    self.criterionRec(self.rec_B_inst, self.real_B_inst) * lambda_rec
                )
                ...
                self.loss_G_inst = (
                    ...
                    + self.loss_cycle_A_inst
                    + self.loss_cycle_B_inst
                    + self.loss_G_A_rec_inst
                    + self.loss_G_B_rec_inst
                    + self.loss_G_A_fake_rec_inst
                    + self.loss_G_B_fake_rec_inst
                    + self.loss_A_rec_inst
                    + self.loss_B_rec_inst
                )
    ...
    def optimize_parameters(self):
        ...
        if (
            ...
            self.set_requires_grad(
                [self.netD_rec_fake_A_inst, self.netD_rec_fake_B_inst], True
            )
