...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu=0, transfer=False):
    net = None
    ...
    if netG == "resnet_9blocks":
        ...
        net = UnetGenerator(
            input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout, transfer=transfer
        )
    elif netG == "unet_256":
        net = UnetGenerator(
            input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout, transfer=transfer,
        )
    else:
...
def define_D(
    ...
    if netD == "basic":  # default PatchGAN classifier
        ...
        net = MultiscaleDiscriminator(
            input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_layer, num_D=num_D
        )
    else:
        raise NotImplementedError(
...
class DemodulatedConv2d(nn.Module):
    ...
    def forward(self, input):
        ...
        weight = self.weight * demod.repeat([batch, 1]).view(
...
class ResnetGenerator(nn.Module):
    ...
    Includes an Encoder, Decoder, and a Reconstructor.
    """
    ...
    def __init__(
        ...
        if type(norm_layer) == functools.partial:
            ...
            if (
                ...
                and norm_layer.func != nn.BatchNorm2d
            ):
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(
        self, dim, padding_type, norm_layer, use_dropout, use_bias, dilation=1
    ):
        ...
        conv_block += [
            Conv2d(
                dim, dim, kernel_size=3, padding=p, bias=use_bias, dilation=dilation
            ),
            norm_layer(dim),
            nn.ReLU(True),
        ]
...
class DomainClassifier(nn.Module):
    ...
    def __init__(self, ngf, norm_layer=nn.InstanceNorm2d):
        ...
        sequence = [
            nn.Conv2d(ngf * 4, ngf * 4, kernel_size=3, stride=2),
            norm_layer(ngf * 4),
            nn.LeakyReLU(0.2, True),
        ]
        sequence += [
            nn.Conv2d(ngf * 4, ngf * 2, kernel_size=3, stride=2),
            norm_layer(ngf * 2),
            nn.LeakyReLU(0.2, True),
        ]
        sequence += [
            nn.Conv2d(ngf * 2, ngf * 2, kernel_size=3, stride=2),
            norm_layer(ngf * 2),
            nn.LeakyReLU(0.2, True),
        ]
